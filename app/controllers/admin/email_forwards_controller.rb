...
class Admin::EmailForwardsController < Admin::ApplicationController
  ...
  def create
    ...
    if @email_forward.valid?
      flash[:notice] = "Par \"#{@email_forward.from}@outwardbound.hr #{"".prepend_icon("arrow-right")} #{@email_forward.to}\" je uspješno dodan."
  ...
  def destroy
    if EmailForward.exists?(params[:id])
      ...
      flash[:notice] = "Par \"#{email_forward.from}@outwardbound.hr #{"".prepend_icon("arrow-right")} #{email_forward.to}\" je uspješno izbrisan."
    end
    redirect_to action: :index
  end
